...
def main(argv=None):
   ...
   # print(sess.run(tower_grads))
    ...
    #print(tf.global_variables(), max_to_keep=500)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        data_generator = icdar2.get_batch(num_workers=FLAGS.num_readers,
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step % FLAGS.save_checkpoint_steps == 0:
                #print(global_step)
                if tl < 0.0018:
                    saver.save(sess, FLAGS.checkpoint_path + str(ml) + '-' + 'model.ckpt', global_step=global_step)
