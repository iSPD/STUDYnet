...
public class PeerConnectionClient {
  ...
  private static final int HD_VIDEO_HEIGHT = 960;
  ...
  @Nullable
  private DataChannel dataChannel;//touch data...
  private DataChannel dataChannelForAIScore;//aiscore data...
  private DataChannel dataChannel2_1;//finger data...
  private DataChannel dataChannel2_2;//finger data...
...
//  private glEngine mGlEngine;
  ...
  public PeerConnectionClient(Context appContext, EglBase eglBase,
    ...
    //khkim 20210316
//    mGlEngine = new glEngine(appContext, 1280, 960, null, null);
  }
  ...
  private void createPeerConnectionInternal() {
    ...
    if (dataChannelEnabled) {
      ...
      dataChannel = peerConnection.createDataChannel("TouchData", init);

      dataChannelForAIScore = peerConnection.createDataChannel("AIScore", init);

      DataChannel.Init init2 = new DataChannel.Init();
      init2.ordered = peerConnectionParameters.dataChannelParameters.ordered;
      init2.negotiated = peerConnectionParameters.dataChannelParameters.negotiated;
      init2.maxRetransmits = peerConnectionParameters.dataChannelParameters.maxRetransmits;
      init2.maxRetransmitTimeMs = peerConnectionParameters.dataChannelParameters.maxRetransmitTimeMs;
      init2.id = peerConnectionParameters.dataChannelParameters.id;
      init2.protocol = peerConnectionParameters.dataChannelParameters.protocol;
      dataChannel2_1 = peerConnection.createDataChannel("HandData1", init2);
      dataChannel2_2 = peerConnection.createDataChannel("HandData2", init2);

      dataChannel3 = peerConnection.createDataChannel("CropData", init2);
    }
    isInitiator = false;
  ...
  private void closeInternal() {
    ...
    if (dataChannel != null) {
      ...
      dataChannelForAIScore.dispose();
      dataChannelForAIScore = null;

      dataChannel2_1.dispose();
      dataChannel2_1 = null;

      dataChannel2_2.dispose();
      dataChannel2_2 = null;
  ...
  public void setVideoSwitch(int texID, boolean useOES)
  {
    executor.execute(() -> {
      try {
        videoCapturer.stopCapture();
      ...
      videoCapturer.setInputTexture(texID, useOES);

      if (useOES == true) {
        videoCapturer.startCapture(videoWidth, videoHeight, videoFps/*15*/);
      } else {
        videoCapturer.startCapture(videoWidth, videoHeight, videoFps);
      }
    });
...
//    localVideoTrack.addSink(localRender);
  ...
  //related touch message
  public void sendTextForAIScore(String data, String type){

    //https://www.programcreek.com/java-api-examples/?code=angellsl10/react-native-webrtc/react-native-webrtc-master/android/src/main/java/com/oney/WebRTCModule/PeerConnectionObserver.java
    if (dataChannelForAIScore != null) {
      byte[] byteArray;
      if (type.equals("text")) {
        try {
          byteArray = data.getBytes("UTF-8");
      ...
      ByteBuffer byteBuffer = ByteBuffer.wrap(byteArray);
      DataChannel.Buffer buffer = new DataChannel.Buffer(byteBuffer, type.equals("binary"));
      dataChannelForAIScore.send(buffer);
    }
  }

  public void sendText2_1(float data[], String type){

    if (dataChannel2_1 != null) {

//      for(int i = 0; i < 21*3; i++) {
//        Log.d(TAG, i+" sendText2_1 : "+data[i]);
//      }
      ...
      DataChannel.Buffer buffer = new DataChannel.Buffer(byteBuffer, type.equals("binary"));
      dataChannel2_1.send(buffer);
    }
  }

  public void sendText2_2(float data[], String type){

    if (dataChannel2_2 != null) {

//      for(int i = 0; i < 21*3; i++) {
//        Log.d(TAG, i+" sendText2 : "+data[i]);
//      }
      ...
      DataChannel.Buffer buffer = new DataChannel.Buffer(byteBuffer, type.equals("binary"));
      dataChannel2_2.send(buffer);
    }
  }

  public void sendText3(float data[], String type){
  ...
  private class PCObserver implements PeerConnection.Observer {
    ...
    public void onIceConnectionChange(final IceConnectionState newState) {
      executor.execute(() -> {
        Log.d(TAG, "[disconnect-debug] IceConnectionState: " + newState);
    ...
    public void onConnectionChange(final PeerConnectionState newState) {
      executor.execute(() -> {
        Log.d(TAG, "[disconnect-debug] PeerConnectionState: " + newState);
