...
public abstract class ImageClassifier {
   ...
   */
  public static ImageClassifier create(Activity activity, Model model, Device device, int numThreads,
                                       String modelPath, String labelPath)
      throws IOException {
    if (model == Model.QUANTIZED) {
      return new ImageClassifierQuantizedMobileNet(activity, device, numThreads, modelPath, labelPath);
    } else if (model == Model.FLOAT) {
      return new ImageClassifierFloatMobileNet(activity, device, numThreads, modelPath, labelPath);
    } else {
  ...
  protected ImageClassifier(Activity activity, Device device, int numThreads,
                            String modelPath, String labelPath) throws IOException {
  ...
  private static List<Recognition> getTopKProbability(Map<String, Float> labelProb) {
    ...
    //khkim modified 20210527
    int index = 0;
    for (Map.Entry<String, Float> entry : labelProb.entrySet()) {
      //pq.add(new Recognition("" + entry.getKey(), entry.getKey(), entry.getValue(), null));
      pq.add(new Recognition("" + index, entry.getKey(), entry.getValue(), null));
      index++;
    }
